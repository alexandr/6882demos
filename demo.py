...
#        print "RAND", rand
    ...
    def plot(self):
        plt.figure()
        plt.hlines(1,0,1)
        plt.eventplot(self.cutoffs, orientation='horizontal', colors='b')
        plt.show()
...
if __name__ == "__main__":
    ...
    mu_sig = float(sys.argv[2])
    cluster_sig = float(sys.argv[3])
    sleep = float(sys.argv[4])

    dp = DirichletProcess(alpha,
            ([0,0], [[mu_sig, 0], [0, mu_sig]]),
            [[cluster_sig, 0], [0, cluster_sig]])

    dp.keep_plotting(1000, sleep=sleep)
